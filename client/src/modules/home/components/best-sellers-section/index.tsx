...
import LaunchCard from "@modules/home/components/launch-section/launch-card"

const COLLECTION_HANDLE = "mais-vendidos"
const PRODUCT_LIMIT = 4

async function BestSellersContent({ region }: { region: HttpTypes.StoreRegion }) {
  const t = await getT()
    ...
    <section className="w-full border-t border-line">
      <div className="content-container py-16 small:py-24">
        <div className="mb-10 small:mb-14 flex items-baseline justify-between">
          <div>
            <p className="eyebrow mb-3">{t("home.bestSellers.eyebrow")}</p>
          ...
          {products.map((product, index) => {
            ...
            const badge =
              typeof product.metadata?.home_badge === "string"
                ...
                : ""
            const typeValue =
              (product.type as { value?: string } | null | undefined)?.value
        ...
        <div className="mt-10 flex justify-center small:hidden">
          ...
          >
            {t("home.bestSellers.viewAllMobile")}
          </LocalizedClientLink>
        </div>
      </div>
    </section>
...
export default async function BestSellersSection({
  ...
  return (
    <Suspense fallback={null}>
      <BestSellersContent region={region} />
    </Suspense>
  )
}